...
function getWebviewContent(prismCssUri: vscode.Uri, prismJsUri: vscode.Uri) {
    return `<!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
    ...
    <body>
        ...
        <script>
            ...
            function renderMessage(message) {
                ...
                const htmlContent = message.content.replace(codeRegex, (match, lang, code) => {
                    const language = lang || 'javascript';
                    return \`<pre><code class="language-\${language}">\${escapeHtml(code.trim())}</code></pre>\`;
                });
                return \`<div class="message \${message.role}">\${htmlContent}</div>\`;
            }
            ...
            window.addEventListener('message', event => {
                ...
                switch (message.type) {
                    case 'addMessage':
                        ...
                        chatContainer.innerHTML += renderMessage(message.message);
                        highlightCode();
                        break;
                }
            });
...
// Function to update the global state outside activate
