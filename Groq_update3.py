# stores embedding in pkl and additional generated summary 
...
if not groq_api_key:
    ...
    # New: Function to summarize repository content using Groq API
    def summarize_repository(directory):
        supported_extensions = ['.java', '.js', '.py', '.php', '.cpp']
        context = ""
        for root, dirs, files in os.walk(directory):
            for file in files:
        ...
        if context:
            ...
            st.error("No valid files found in the directory to summarize.")
    ...
    # New: Button to summarize repository
    if directory and st.button("Generate Summary"):
        summarize_repository(directory)
