# storing the embeddings to local storage in current directory
# .json is created
# loading vectors and reteriving from this local .json file when needed. 
...
if not groq_api_key:
    ...
    # Save embeddings to local storage as JSON
    def save_embeddings_to_file(embeddings, text_chunks_store, file_path='embeddings.json'):
        data = []
        # Prepare data structure to be saved in JSON
        for i, (embedding, chunk_data) in enumerate(zip(embeddings, text_chunks_store)):
            data.append({
    ...
    def load_embeddings_from_file(file_path='embeddings.json'):
        ...
        text_chunks_store = []
        for item in data:
            embeddings.append(np.array(item['embedding']))  # Convert list back to numpy array
    ...
    def process_repository(directory):
        ...
        # Save all embeddings and text chunks to JSON
